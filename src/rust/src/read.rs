...
///
/// @export
pub fn info(filename: String, encryption_key: Option<String>) ->
    ...
    if let Some(key) = encryption_key.as_ref() {
        ...
        dec_props.insert(MzPeakArchiveType::SpectrumDataArrays
            .tag_file_suffix().to_string(), dec.clone());
        dec_props.insert(MzPeakArchiveType::SpectrumMetadata
            .tag_file_suffix().to_string(), dec.clone());
        dec_props.insert(MzPeakArchiveType::SpectrumPeakDataArrays
            .tag_file_suffix().to_string(), dec.clone());
        dec_props.insert(MzPeakArchiveType::ChromatogramDataArrays
            .tag_file_suffix().to_string(), dec.clone());
        dec_props.insert(MzPeakArchiveType::ChromatogramMetadata
            .tag_file_suffix().to_string(), dec.clone());
        dec_props.insert(MzPeakArchiveType::WavelengthSpectrumMetadata
            .tag_file_suffix().to_string(), dec.clone());
        dec_props.insert(MzPeakArchiveType::WavelengthSpectrumDataArrays
            .tag_file_suffix().to_string(), dec.clone());
    }
...
pub fn mzpeak_n_spectra(filename: String, encryption_key: Option<String>) ->
                extendr_api::Result<usize> {
    let mut dec_props = HashMap::default();
    if let Some(key) = encryption_key.as_ref() {
        let dec = FileDecryptionProperties::builder(key.as_bytes().to_vec())
            .build()
